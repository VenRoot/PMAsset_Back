...
const ad = new ActiveDirectory({
    url: "ldap://azrweudc01.jumbo.net",
    baseDN: "dc=jumbo,dc=net",
...
export const getAllUsers = () => new Promise((resolve, reject) => {
    ...
    const attributes = [
        ...
        'userPrincipalName', //Markus.Loeffler@putzmeister.com
        // 'distinguishedName', 
        // 'objectCategory', 
        'cn', //Markus Löffler
        // 'description', 
        ...
        // 'sn', //Löffler
        // 'givenName', //Markus
        // 'displayName', //Markus Löffler (selbe wie cn)
    ...
    ad.findUsers(opt, (err, user:users[]) => {
        ...
        // resolve(user.filter(us => (us.memberOf as string[])?.includes("CN=All Users - Putzmeister,OU=DE,OU=Groups,OU=RealmJoin,OU=OneIT,DC=jumbo,DC=net")) as any);
        // resolve(user as any);
    });
});
